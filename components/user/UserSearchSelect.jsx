...
class UserSearchSelect extends React.Component {
    ...
    render() {
        const userItems = this.props.filteredUsers && this.props.filteredUsers.length > 0 ? this.props.filteredUsers.map(user => (
            <div className=" flex justify-between my-2" key={user._id}>
                <UserAvatar user={user}/>
                <span className="pt-2">{user.firstname} {user.lastname}</span>
                <AddButton className="mt-1" onClick={(e)=>{this.props.onSelect(e,user)}}/>

            </div>
        )) : (<span>No User found</span>)
        return <div className={"user-search-wrapper relative " + this.props.className} onClick={this.props.onClick}>
            <div className="field">
                <div className="control">
                    <input className="input" type="text" placeholder="Search for name" value={this.props.value}
